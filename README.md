...
- `Watch Sass...` 꼭 끄셔야 불필요한 css 생성안됩니다.
...
const Slide = () => {
  ...
  return (
    <div style={{ padding: "30" }}>
      <h1 style={{ textAlign: "center" }}>CSS-inJS 예제</h1>
      ...
      <br />
      <br />
      <Button size="lg">기본버튼 : large</Button>
      <Button>기본버튼</Button>
    </div>
  );
};
...
- 3차 버전 (src/components/ui/Button.jsx)
...
- JSX 에서 `JS 자리`에 if 문과 switch 문을 사용할 수 있음.

- JSX 에서 `JSX 자리`에 if 문과 switch 문을 사용할 수 없음.
- if 문과 switch 문 대신에 `3항 연산자`가 가장 많이 사용됨.

- JSX 에서 `JSX 자리`에 falshy 한 값은 출력이 안됩니다.
...
function Test() {
  ...
  // jsx
  return (
    <Hi isLogin={true} msg={"새로운메시지"} user={{ name: "홍길동", age: 20 }}>
      Test
    </Hi>
  );
}
