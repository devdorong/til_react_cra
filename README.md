...
```txt
http://localhost:3000/                          첫페이지, 홈페이지, index.html
http://localhost:3000/about                     소개
http://localhost:3000/about/mission             미션
http://localhost:3000/about/team                팀 소개
-----------------------------------------------------------------------------
...
http://localhost:3000/blog/design/detail?id=1   쿼리
```

## 4. Router 에 맞게 pasges 폴더를 구성하자.
...
----------------------------------------------------------------------------------
http://localhost:3000/blog/design/1             src/pages/blog/detail/[id].jsx
...
```jsx
<Header company={"좋은회사"} service={"Todo 서비스"}></Header>
```

### 7.7 컴포넌트에 Props 인 Children 전달하기

```jsx
<Header company={"좋은회사"} service={"Todo 서비스"}>
  <div>🤦‍♂️ 나는 자식입니다.</div>
  <div>🤦‍♀️ 자식은 여려명이 가능합니다.</div>
</Header>
```
...
<img width="702" height="189" alt="Image" src="https://github.com/user-attachments/assets/4c1dae05-d9fe-4b08-8978-73e0f2084a39" />
...
### 7.8 레이아웃을 유지하면서 Outlet 에 출력하기

- Route 는 오로지 컴포넌트 이동 후 출력만 담당
- Route 는 레이아웃 역할을 하지 못합니다.
- Route 를 레이아웃 용도로 활용하겠다는것
...
function Layout() {
  return (
    <div>
      ...
      <div>
        <h2>Outlet 자리</h2>
        <div style={{ background: "yellow", minHeight: "100px" }}>
          <Outlet />
        </div>
      </div>
    </div>
...
- params나 QueryString은 사용자에게 보여집니다.
